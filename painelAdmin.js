// Substitua 'VITE_API_URL' por uma URL fixa (ex: http://localhost:3000)
const API_URL = "http://localhost:3000"; // Ajuste conforme seu backend
  ...
  .addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/usuarios/cadastrar-treinador`,
        {
          method: "POST",
          headers: {
      ...
      showModalAlert("Erro", "Erro ao conectar com o servidor.");
    }
  });
...
async function carregarTreinadores() {
  ...
  try {
    const res = await fetch(`${API_URL}/api/usuarios/treinadores`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    showModalAlert("Erro", "Erro ao carregar treinadores.");
  }
}
  ...
  if (confirmado) {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/usuarios/remover-treinador/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      showModalAlert("Erro", "Erro ao remover treinador.");
    }
  }
}
...
document
  .getElementById("editTrainerForm")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/usuarios/editar-treinador/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      showModalAlert("Erro", "Erro ao editar treinador.");
    }
  });
